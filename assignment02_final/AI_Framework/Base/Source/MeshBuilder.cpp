...
Mesh* MeshBuilder::GenerateBeam(const std::string &meshName, Color color, float length)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;

	v.pos.Set(0.f * length, 0.f * length, 0.f * length);
	v.color = color;
	vertex_buffer_data.push_back(v);
	v.pos.Set(1.f * length, 0.f * length, 0.f * length);
	v.color = color;
	vertex_buffer_data.push_back(v);
	v.pos.Set(1.f * length, 1.f * length, 0.f * length);
	v.color = color;
	vertex_buffer_data.push_back(v);
	v.pos.Set(0.f * length, 1.f * length, 0.f * length);
	v.color = color;
	vertex_buffer_data.push_back(v);
	v.pos.Set(0.f * length, 0.f * length, 1.f * length);
	v.color = color;
	vertex_buffer_data.push_back(v);
	v.pos.Set(1.f * length,0.f * length,1.f * length);
	v.color = color;
	vertex_buffer_data.push_back(v);
	v.pos.Set(1.f * length, 1.f * length,1.f * length);
	v.color = color;
	vertex_buffer_data.push_back(v);
	v.pos.Set(0.f * length, 1.f * length, 1.f * length);
	v.color = color;
	vertex_buffer_data.push_back(v);
...
bool MeshBuilder::HasMesh(const std::string& _meshName)
{
	return meshMap.count(_meshName) != 0;
}